    ...
    def render_patchv4(self, img, gt_semantic_seg):
        ...
        for i in range(batchsize):
            ...
            for _ in range(num_patch):
                ...
                img[i], gt_semantic_seg[i], ood_seg[i], corrdinate_i = self.randout(hulldict, gt_semantic_seg[i], ood_seg[i], patch, self.patch_cfg["patch_index"],self.decode_head.ignore_index)
    ...
    def forward_train(self, img, img_metas, gt_semantic_seg):
        ...
        for i in range(img.size(0)):
            for j in range(num_patch):
                img[i, :, corrdinate[i][j][0]+corrdinate[i][j][2], corrdinate[i][j][1]+corrdinate[i][j][3]] = img[i, :, corrdinate[i][j][0]+corrdinate[i][j][2], corrdinate[i][j][1]+corrdinate[i][j][3]] + 5 * perturbe[i, :, corrdinate[i][j][0]+corrdinate[i][j][2], corrdinate[i][j][1]+corrdinate[i][j][3]]
        """
        self.add_patch(img, gt_semantic_seg)
        ...
        for i in range(4):
            ...
            x = self.extract_feat(img_new)
            loss_decode = self._decode_head_forward_train(x, img_metas_new, gt_semantic_seg_new)
            ...
            with torch.no_grad():
                ...
                ood_seg = self._refine_ood_gt(ood_score, ood_seg)
            loss_ood,_ = self._ood_head_forward_train(ood_inp, img_metas_new, ood_seg, ood_score)
            
            if self.with_auxiliary_head:
                loss_aux = self._auxiliary_head_forward_train(
                    x, img_metas_new, gt_semantic_seg_new)
            if loss_ood['ood.loss_ce']>loss_ood_f['ood.loss_ce']:
               loss_decode_f, loss_ood_f, loss_aux_f = loss_decode, loss_ood, loss_aux          
