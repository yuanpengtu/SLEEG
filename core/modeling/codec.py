...
import torchvision.transforms as transformsnew
...
class PatchEncoderDecoder(BaseOODEncoderDecoder):
    ...
    def __init__(self,
        ...
        self.gaussian_smoothing = transformsnew.GaussianBlur(7, sigma=1)
    ...
    def render_patchvoid(self, img, gt_semantic_seg):
        ood_seg = gt_semantic_seg.new_zeros(gt_semantic_seg.size())
        gt_semantic_seg = gt_semantic_seg.clone()
        ood_seg = (gt_semantic_seg==self.decode_head.ignore_index).long()
        return img, gt_semantic_seg, ood_seg, 1, None
    ...
    def forward_train(self, img, img_metas, gt_semantic_seg):
        ...
        #img, gt_semantic_seg, ood_seg, num_patch, corrdinate = self.render_patchvoid(img, gt_semantic_seg)#self.render_patchv4(img, gt_semantic_seg)
        ...
        perturbe = ag.grad(loss_logit, inputs=[mask_adv], retain_graph=True)[0]
        perturbe = perturbe.detach()        
        perturbe[perturbe>0] = 1
        perturbe[perturbe<0] = -1
        for i in range(img.size(0)):
            for j in range(num_patch):
                img[i,:,:,:] = img[i,:,:,:] + perturbe[i,:,:,:] * 0.000005
                img[i, :, corrdinate[i][j][0]+corrdinate[i][j][2], corrdinate[i][j][1]+corrdinate[i][j][3]] = img[i, :, corrdinate[i][j][0]+corrdinate[i][j][2], corrdinate[i][j][1]+corrdinate[i][j][3]] + 0.00006 * perturbe[i, :, corrdinate[i][j][0]+corrdinate[i][j][2], corrdinate[i][j][1]+corrdinate[i][j][3]]
    ...
    def _get_ood_segmentation_score(
        ...
        #out_ood = self.gaussian_smoothing(out_ood)
        return out_ood
